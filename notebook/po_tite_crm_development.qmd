...
get_next_dose_po_tite_crm <- function(
  ...
  if (est_p[1] <= target_p) {
    next_dose <- po[max(which(est_p <= target_p))]
  } else {
    warning("Min dose below target")
    stop()
  }
...
begin_sim <- function(
  ...
  while (is_step1_phase & patient_data$n_enrolled < 25) {
    ...
    if (length(unique(response)) > 1) {
      ...
      if (zone < 5) {
        zone <- zone + 1
        next_dose <- sample(rep(treatments_by_zone[[zone]], 2), 1)
      } else {
        # stop trial, return patients data
        if (verbose) print("No toxicity observed until the end of trial")
        return(patient_data)
      }
    }
  }
  ...
  while (patient_data$n_enrolled < 25) {
    patient_data <- enroll_patient(
      patient_data, 
      enrollment_date = current_date, 
      assigned_dose = next_dose,
...
# following is equivalent to MTD
get_next_dose_po_tite_crm(
    patient_data = patient_data,
    current_date = 1000,
    po_mat = po_mat,
    skeleton_mat = skeleton_mat
